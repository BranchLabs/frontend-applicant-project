...
function reducer(state, action) {
	...
	switch (type) {
		...
		case 'ARROW_KEY_PRESS': {
			...
			if (code === 37 && x !== 0) {
				return { ...state, coordinates: [--x, y] }; // Left arrow press
			} else if (code === 38 && y !== 0) {
				return { ...state, coordinates: [x, --y] }; // Up arrow press
			} else if (code === 39 && x !== width - 1) {
				return { ...state, coordinates: [++x, y] }; // Right arrow press
			} else if (code === 40 && y !== height - 1) {
				return { ...state, coordinates: [x, ++y] }; // Down arrow press
			}
