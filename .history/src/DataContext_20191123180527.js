...
function DataProvider({ children }) {
	...
	const [state, dispatch] = React.useReducer(reducer, {
		...
		tableData: [
			['Kiril', '->', 'BranchLabs'],
			[1, 2, 3],
			['React Hooks', '=SUM(A1:A2)', '=SUM(A1, A1)'],
		],
	});
	return (
...
Excel.on('callCellValue', function(cellCoord, done) {
	...
	console.log('tableDat2a', context);
...
function updateCell(dispatch, table, x, y, value) {
	if (value && value.charAt(0) == '=') {
		console.log('tableDat2a', table);
		//value = Excel.parse(value.slice(1));

		Excel.on('callCellValue', function(cellCoord, done) {
			// using label
			console.log('cellCoord', cellCoord);
			let height = cellCoord.row.index;
			let width = cellCoord.column.index;
			done(get(table, [height][width], 0));
		...
		value = Excel.parse(value.substring(1)); // It returns `Object {error: null, result: 14}`
	}
