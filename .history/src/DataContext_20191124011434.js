...
function reducer(state, action) {
	...
	switch (type) {
		...
		case 'ARROW_KEY_PRESS': {
			...
			// Second function is preventing out-of-bounds navigation
			if (code === 37 && x !== 0) {
				coordinates = [--x, y]; // Left arrow press
			} else if (code === 38 && y !== 0) {
				coordinates = [x, --y]; // Up arrow press
			} else if (code === 39 && x !== width - 1) {
				coordinates = [++x, y]; // Right arrow press
			} else if (code === 40 && y !== height - 1) {
				coordinates = [x, ++y]; // Down arrow press
			}
