...
function DataProvider({ children }) {
	...
	const [state, dispatch] = React.useReducer(reducer, {
		...
		tableData: [
			['Kiril', '->', 'BranchLabs'],
			[1, 2, 3],
			['React Hooks', '=SUM(A1: A3)', '=SUM(A2, A2)'],
		],
	});
	return (
...
function updateCell(dispatch, table, x, y, value) {
	if (value && value.charAt(0) == '=') {
		...
		Excel.on('callRangeValue', function(startCellCoord, endCellCoord, done) {
			...
			console.log('startCell', startCellCoord);
			console.log('endCell', endCellCoord);
			console.log(`Iterating ${startCellCoord.column.index} to ${endCellCoord.column.index}`);
			let collection = [];
			// Iterate over rows first
			for (c = startCellCoord.column.index; c < endCellCoord.column.index; c++) {
				console.log('23', c);
				for (r = startCellCoord.row.index; r < endCellCoord.row.index; r++) {
					console.log(get(table, `[${c}][${r}]`, 0));
				}
			}
		});
