...
function reducer(state, action) {
	...
	switch (type) {
		case 'LOAD_DATA': {
			...
			let interpretedData = tableData.map(function(tableRow) {
				tableRow.map((value, index) => {
					console.log('value', value);
					let normalizedValue = value.length > 0 ? parseFomula(tableData, value) : '';
				});
			});
			return { ...state, tableData, size, coordinates: [0, 0] };
		}
