...
function Table(props) {
  ...
  for (let y = 0; y < props.height + 1; y += 1) {
    ...
    if (y == 0) {
      ...
      rows.push(<Row key={-1} className='read-only' rowContent={new Array(width + 1).fill().map((value, index) => String.fromCharCode(index + 65))} />);
    }
    // Individually return groups of <tr> objects
    rows.push(<Row key={y} className='table-data' y={y} rowContent={[2, 3]} />);
  }
