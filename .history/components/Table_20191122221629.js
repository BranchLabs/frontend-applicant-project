...
function Table({ width, height }) {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    if (y == 0) rows.push(<Row key={-1} readOnly={true} rowContent={new Array(width + 1).fill().map((value, index) => String.fromCharCode(index + 65))} />);
    // Individually return groups of <tr> objects
    rows.push(<Row key={y} y={y} rowContent={get(context.tableData, y, [])} />);
  }
