...
function Cell({ x, y, readOnly, content }) {
	...
	useEffect(() => {
		...
		console.log('x', x);
		console.log('current', coordinates[0]);
		console.log('selection', selection_coordinates[0]);
	...
	return (
		<TableData selected={selected}>
			<CellContent
				...
				onMouseOver={e => {
					setHovering(true);
					if (!multiple) tableDispatch({ type: 'SET_SELECTION_END', x, y });
				}}
				onMouseOut={e => {
					setHovering(false);
				}}
				onMouseUp={() => {
					if (hovering) {
						tableDispatch({ type: 'SET_SELECTION_END', x, y });
						console.log('moused over', [x, y]);
					}
					setMultiple(false);
				}}
				ref={ref}
